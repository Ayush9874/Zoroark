...
export function BrainNode({ data }: any) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "relative px-6 py-4 rounded-xl border-2 transition-all duration-500 min-w-[220px]",
        status === 'running' ? "bg-blue-500/10 border-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.2)]" : 
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] font-mono uppercase tracking-widest text-zinc-500">Brain / LLM</span>
        </div>
      ...
      {status === 'done' && data.result && (
        ...
        >
          <div className="text-[10px] font-mono text-emerald-400/80 uppercase mb-1">Thought</div>
          <div className="text-[11px] text-emerald-200/90 leading-tight italic">"{data.result}"</div>
        </motion.div>
      )}
